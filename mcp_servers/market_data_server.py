...
class MarketDataProvider:
        ...
        try:
            ...
            return {
                ...
                "macd": {
                    ...
                    "signal_line": (
                        float(macd_signal) if not pd.isna(macd_signal) else None
                    ),
                    "histogram": (
                        float(macd_histogram) if not pd.isna(macd_histogram) else None
                    ),
                },
                "bollinger_bands": {
                ...
                "signals": self._generate_trading_signals(
                    current_price, ma_20, ma_50, rsi, macd_line, macd_signal
                ),
                "timestamp": datetime.now().isoformat(),
            }
        except Exception as e:
    ...
    def _calculate_bollinger_bands(
        self, prices: pd.Series, window: int = 20, num_std_dev: int = 2
    ) -> tuple:
...
async def get_portfolio_performance(request: PortfolioRequest):
    ...
    try:
        ...
        if len(symbols) != len(weights):
            raise HTTPException(
                status_code=400, detail="Number of symbols must match number of weights"
            )
