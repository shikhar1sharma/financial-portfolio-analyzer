...
class PortfolioManager:
    ...
    def add_position(
        ...
        try:
            ...
            cursor.execute(
                "SELECT id, quantity, purchase_price FROM positions WHERE symbol = ?",
                (symbol.upper(),),
            )
            ...
            if existing_position:
                ...
                new_avg_price = (
                    (old_quantity * old_price) + (quantity * purchase_price)
                ) / new_quantity
        ...
        try:
            ...
            for position in positions:
                ...
                try:
                    ...
                    hist = ticker.history(period="1d")
                    current_price = (
                        hist["Close"].iloc[-1] if not hist.empty else purchase_price
                    )
                    ...
                    self.logger.warning(
                        f"Could not fetch current price for {symbol}: {e}"
                    )
...
async def root():
    return {
        "message": "Portfolio Management Server",
        "status": "running",
        "timestamp": datetime.now().isoformat(),
    }
...
async def add_stock_position(request: AddPositionRequest):
    ...
    return portfolio_manager.add_position(
        request.symbol, request.quantity, request.purchase_price, request.purchase_date
    )
